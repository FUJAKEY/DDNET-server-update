...
void CMapRenderer::Load(ERenderType Type, CLayers *pLayers, IMapImages *pMapImages, IEnvelopeEval *pEnvelopeEval, std::optional<FRenderUploadCallback> FRenderCallbackOptional)
{
	Clear();
	...
	for(int GroupId = 0; GroupId < pLayers->NumGroups(); GroupId++)
	{
		...
		pRenderLayerGroup->OnInit(Graphics(), TextRender(), RenderMap(), pEnvelopeEval, pLayers->Map(), pMapImages, pEnvelopePoints, FRenderCallbackOptional);
		if(!pRenderLayerGroup->IsValid())
		{
			dbg_msg("map_renderer", "error group was null, group number = %d, total groups = %d", GroupId, pLayers->NumGroups());
			dbg_msg("map_renderer", "this is here to prevent a crash but the source of this is unknown, please report this for it to get fixed");
			dbg_msg("map_renderer", "we need mapname and crc and the map that caused this if possible, and anymore info you think is relevant");
			continue;
		}
		...
		for(int LayerId = 0; LayerId < pGroup->m_NumLayers; LayerId++)
		{
			...
			if(pRenderLayer)
			{
				pRenderLayer->OnInit(Graphics(), TextRender(), RenderMap(), pEnvelopeEval, pLayers->Map(), pMapImages, pEnvelopePoints, FRenderCallbackOptional);
