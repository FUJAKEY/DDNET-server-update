...
void CGameContext::ConAuthor(IConsole::IResult *pResult, void *pUserData)
{
	...
	{
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "author", "Invalid or offline target client id.");
		return;
	}
	...
	pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "author", aBuf);
}
...
void CGameContext::ConAddMoney(IConsole::IResult *pResult, void *pUserData)
{
	...
	if(Victim >= 0)
		TargetCid = Victim;
	else if(pResult->m_ClientId >= 0)
		TargetCid = pResult->m_ClientId;
	else
	{
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "addmoney", "Specify target id when using from rcon.");
		return;
	}
	
	if(TargetCid < 0 || TargetCid >= MAX_CLIENTS || !pSelf->m_apPlayers[TargetCid] || !pSelf->Server()->ClientIngame(TargetCid))
	{
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "addmoney", "Invalid or offline target client id.");
		return;
	}
